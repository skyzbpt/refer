  ...
  const result = await Promise.all(users.results.map(async (u) => {
    ...
    try {
      refs = await env.DB.prepare(
        'SELECT id, friend, service, date, contact, message FROM referrals WHERE user_uid = ? ORDER BY id ASC'
      ).bind(u.uid).all();
    } catch (e) {
      // 資料庫尚未加入 contact / message 欄位時的相容查詢
      refs = await env.DB.prepare(
        'SELECT id, friend, service, date FROM referrals WHERE user_uid = ? ORDER BY id ASC'
      ).bind(u.uid).all();
    }

    const ledger = await env.DB.prepare(
      'SELECT id, type, amount, note, date FROM ledger WHERE user_uid = ? ORDER BY id ASC'
    ).bind(u.uid).all();
